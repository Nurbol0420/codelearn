...
class _LessonListState extends State<LessonList> {
            ...
            children: [
              ...
              if (isExpanded)
                ...sectionLessons.asMap().entries.map((entry) {
                  ...
                  // Locked if: premium not purchased (except preview), or prev lesson not done
                  final isPremiumLocked = course.isPremium && !widget.isUnlocked && !lesson.isPreview;
                  final isSequenceLocked = !lesson.isPreview &&
                      globalIdx > 0 &&
                      !_completedLessons.contains(course.lessons[globalIdx - 1].id);
                  final isLocked = isPremiumLocked || isSequenceLocked;
  ...
  Future<void> _onLessonTap(Course course, lesson, bool isLocked) async {
    // Premium курс и не куплен — разрешаем только preview уроки
    if (course.isPremium && !widget.isUnlocked && !lesson.isPreview) {
      Get.toNamed(
        AppRoutes.payment,
        arguments: {
          'courseId': course.id,
