...
class InProgressSection extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        if (inProgressCourses.isNotEmpty)
          Text(l10n.inProgress, style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        if (inProgressCourses.isEmpty) const SizedBox.shrink(),
        Column(
          children: inProgressCourses.map((course) {
            final completedLessons = course.lessons.where((lesson) => false).length;
            ...
            return Padding(
              ...
              child: Container(
                ...
                child: Material(
                  ...
                  child: InkWell(
                    borderRadius: BorderRadius.circular(16),
                    onTap: () => _handleInProgressCourseTap(context, course.id, completedLessons),
                    child: Padding(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(course.title, style: theme.textTheme.titleMedium?.copyWith(color: AppColors.primary, fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                Text(
                                  '${l10n.progress}: ${(progress * 100).toInt()}%',
  ...
  void _handleInProgressCourseTap(BuildContext context, String courseId, int lastLesson) {
    Get.toNamed('/course/$courseId', parameters: {'id': courseId, 'lastLesson': lastLesson.toString()});
  }
}
